...
#   OrangeFox-R12.0-Unofficial-chagall-system-compatible.img  (flashable via OTA tools)
#   OrangeFox-R12.0-Unofficial-chagall.img (+ .md5)           (fastboot-friendly copy)
# ============================================================================
set -euo pipefail

DEVICE_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd -P)"
TOP_DIR="$(cd -- "${DEVICE_DIR}/../../.." && pwd -P)"
PRODUCT_OUT="${1:-${OUT_DIR:-${TOP_DIR}/out}/target/product/chagall}"

STOCK_RAMDISK="${DEVICE_DIR}/prebuilt/stock_vendor_ramdisk.cpio.lz4"
DEFAULT_OUTPUT_IMAGE="${PRODUCT_OUT}/OrangeFox-R12.0-Unofficial-chagall-system-compatible.img"
OUTPUT_IMAGE="${2:-${DEFAULT_OUTPUT_IMAGE}}"
STOCK_DTB="${DEVICE_DIR}/prebuilt/dtb/mt6899-chagall.dtb"
RECOVERY_LZ4="${PRODUCT_OUT}/obj/PACKAGING/vendor_ramdisk_fragments_intermediates/recovery.cpio.lz4"
LZ4="${PRODUCT_OUT%/target/product/chagall}/host/linux-x86/bin/lz4"
MKBOOTIMG="${PRODUCT_OUT%/target/product/chagall}/host/linux-x86/bin/mkbootimg"
MKBOOTFS="${PRODUCT_OUT%/target/product/chagall}/host/linux-x86/bin/mkbootfs"
AVBTOOL="${PRODUCT_OUT%/target/product/chagall}/host/linux-x86/bin/avbtool"
DTB_PATCHER="${DEVICE_DIR}/tools/patch-vendor-boot-dtb.py"

work_dir="$(mktemp -d "${TMPDIR:-/tmp}/chagall-vendor-boot.XXXXXX")"
...
patched_dtb="${work_dir}/mt6899-chagall-no-usb-offload.dtb"
...
cp -fp "$OUTPUT_IMAGE" "${PRODUCT_OUT}/OrangeFox-R12.0-Unofficial-chagall.img"
md5sum "${PRODUCT_OUT}/OrangeFox-R12.0-Unofficial-chagall.img" \
    > "${PRODUCT_OUT}/OrangeFox-R12.0-Unofficial-chagall.img.md5"
rm -f \
    "${PRODUCT_OUT}/OrangeFox-R12.0-Unofficial-chagall.zip" \
    "${PRODUCT_OUT}/OrangeFox-R12.0-Unofficial-chagall.zip.md5"
