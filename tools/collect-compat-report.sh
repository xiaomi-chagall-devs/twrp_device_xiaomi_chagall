...
OUTPUT_DIR="${1:-chagall-compat-$(date +%Y%m%d-%H%M%S)}"
IMAGE="${2:-}"
...
for name in \
    ...
    init.svc.chagall-touch-loader init.svc.chagall-touchfeature \
    init.svc.chagall-touch-wait; do
    printf "%s=%s\n" "$name" "$(getprop "$name")"
done'
...
for module in nt38771_touch_chagall xiaomi_touch_chagall scp; do
...
sha256sum /lib/modules/nt38771_touch_chagall.ko \
    /lib/modules/xiaomi_touch_chagall.ko 2>/dev/null || true'
